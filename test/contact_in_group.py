...
def test_del_contact_from_group(app):
    ...
    if len_db_group:
        if db_contacts:
            for i in range(len_db_group):
                ...
                if db.get_contacts_in_group(db_group[i]):
                    ...
                    app.contact.delete_contact_from_group(db_group[i])
                    contacts_not_in_group = db.get_contacts_not_in_groupe(db_group[i])
                    for i in range (len(contacts_not_in_group)):
                        ...
                        if random_contact_id == contact_id_not_in_group:
                           ...
                           print(("Контакт id = "+random_contact_id+" не удалился из группы id = " + group_id_list[i]))
        else:
            ...
            contact_in_group = db.get_contacts_in_group(random_group_id)
            ...
            contacts_not_in_group = db.get_contacts_not_in_groupe(random_group_id)
        ...
        if db_contacts:
                    ...
                    contact_in_group = db.get_contacts_in_group(group_id)
                    contact_id_in_group = contact_in_group[0].id
                    ...
                    contacts_not_in_group = db.get_contacts_not_in_groupe(group_id)
                    contact_id_not_in_group = contacts_not_in_group[0].id
